...
if(isset($_GET['c']) && isset($_GET['s'])){
    $cat_qry = $conn->query("SELECT * FROM categories where id = '{$_GET['c']}'");
    if($cat_qry->num_rows > 0){
        $result =$cat_qry->fetch_assoc();
        $title = $result['category'];
        $cat_description = $result['description'];
    }
 $sub_cat_qry = $conn->query("SELECT * FROM sub_categories where id = '{$_GET['s']}'");
    ...
    <div class="container-fluid row">
        ...
        <div class="col-md-3 border-right mb-2 pb-3">
            ...
            <div class="list-group">
                ...
                <?php 
                $sub_cat = $conn->query("SELECT * FROM `sub_categories` where parent_id =  '{$_GET['c']}' ");
                while($row = $sub_cat->fetch_assoc()):
                ?>
                    <a href="./?p=products&c=<?php echo $_GET['c'] ?>&s=<?php echo $row['id'] ?>" class="list-group-item  <?php echo isset($_GET['s']) && $_GET['s'] == $row['id'] ? "active" : "" ?>"><?php echo $row['sub_category'] ?></a>
                <?php endwhile; ?>
            </div>
            <hr>
        </div>
            ...
            <div class="row gx-2 gx-lg-2 row-cols-1 row-cols-md-3 row-cols-xl-4">
                    ...
                    <?php 
                        ...
                        elseif(isset($_GET['c']) && isset($_GET['s']))
                            $whereData = " and category_id = '{$_GET['c']}' and sub_category_id = '{$_GET['s']}')";
                        elseif(isset($_GET['c']) && !isset($_GET['s']))
                            $whereData = " and category_id = '{$_GET['c']}' ";
                        elseif(isset($_GET['s']) && !isset($_GET['c']))
                            $whereData = " and sub_category_id = '{$_GET['s']}' ";
                    ...
                    ?>
                        <div class="col-xs-12 col-sm-6 col-md-4 col-lg-3 col-xl-3 mb-3 d-flex align-items-stretch">
                            <a class="card product-item text-dark  w-100" href=".?p=view_product&id=<?php echo $row['id'] ?>">
                                <!-- Product image-->
                                <img class="card-img-top w-100" style="max-width: 100%;height:8rem !important"  src="<?php echo validate_image($img) ?>" loading="lazy" alt="..." />
                                <!-- Product details-->
